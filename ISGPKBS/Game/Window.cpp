...
namespace isgp {
	...
	/////////////////////////////////////
	Window::Window() {
		// level handles destruction of grid
		Grid* g = new Grid(g_tiles.GetWidth(),g_tiles.GetHeight());
		_level = new Level(g);
		Point position = Point(5,5);
		g->GetTileAt(position)->SetIpaintalbe(
			new WorldBlock(position, "C:\\smile.bmp"));
	}
